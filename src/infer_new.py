...
def infer_coliee_task3(sentence, all_civil_code, data_args, tfidf_vectorizer, trainer, bert_tokenizer,  tokenizer=None, topk=150):
    ...
    for i, s_pred in tqdm(enumerate(test_pred)):
        for idx in s_pred:
...
def init_state(path_c_code, path_data_org, path_preprocessed_data, model_path, tokenizer=None, topk=150, testing_data=None, max_seq_length=512,
    ...
    print('Transform the data to tfidf vectors, {} no. documents.'.format(len(c_docs)))
    #c_vect = tfidf_vectorizer.transform([standardize_data(d) for d in (c_docs)])
    with open('{}/tfidf_standardize.pkl'.format(path_preprocessed_data), 'rb') as handle:
        c_vect = pickle.load(handle)
    print('Done')
...
def main():
    ...
    model_configs = {
        'PhoBERT': {
            ...
            "do_lower_case": True,
            "max_seq_length": 256,
            "path_preprocessed_data": 'data/zalo-tfidfbm25150-full-pyvi/',
            "model_path": 'settings/Tfbm150E5-full-phobert-pyvi42/models/checkpoint-21500',
        }
    }
    print(json.dumps(model_configs, indent=2))

    test_all_data = json.load(
        open('data/zac2021-ltr-data/private_test_infer.json'))['items']
    ...
    for m_name, model_info in tqdm(model_configs.items()):
        ...
        print("Infer in coliee task 3 successfully")
    ...
    for m_name, model_info in tqdm(model_configs.items()):
        ...
        ic(predicted_labels, probs, c_code_pred_by_tfidf)
    ...
    json.dump(submit_result, open("data/result/result_prediction_vibert_pyvi.json", "wt", encoding='utf8'), ensure_ascii=False, indent=2)
