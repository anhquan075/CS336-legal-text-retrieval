...
def prediction_corpus(test_sents, test_ids):
    missing_ids_info = {}
    real_prediction = {}
    ...
    for m_name, _ in tqdm(model_configs.items()):
        ...
        predicted_labels = [
            x for x in list_split(predicted_labels, topk)]
...
class serviceLegalTextRetrievalHandler(Resource):
    ...
    def post(self):
        ...
        try:
            result = prediction_corpus(test_sents, test_ids)
            ...
            return {'message': 'Successfully', 'result': json.dumps(response)}, 200
